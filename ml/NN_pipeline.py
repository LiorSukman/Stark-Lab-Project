...
def evaluate_predictions(model, data, verbos = True):
    ...
    if verbos:    
        ...
        print('Number of correct classified feature chunks is %d, which is %.4f%%' % (correct_chunks, 100 * correct_chunks / total_chunks))
...
def run(epochs, patience, batch_size, learning_rate, optimizer, n1, n2, f1, f2, classes, features, dataset_path, loading_path, saving_path):
    """
    ...
    trainer = SupervisedTrainer(criterion = criterion, batch_size = batch_size, patience = patience, eval_criterion = eval_criterion, path = saving_path)

    model = Net(n1, n2, f1, f2, features, classes)
    if loading_path == None:
        print('Starting training...')
        best_epoch = trainer.train(model, train_squeezed, num_epochs = epochs, dev_data = dev_squeezed, learning_rate = learning_rate, optimizer = optimizer)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_path', type=str, help='path to the dataset, assume it was created', default = '../data_sets/200_0.60.20.2/')
    parser.add_argument('--loading_path', type=str, help='path to a trained model to evaluate', default = None)
    parser.add_argument('--saving_path', type=str, help='path to save models while training, assumes path exsists', default = '../saved_models/')
    ...
    run(epochs, patience, batch_size, learning_rate, optimizer, n1, n2, f1, f2, classes, features, dataset_path, loading_path, saving_path)
