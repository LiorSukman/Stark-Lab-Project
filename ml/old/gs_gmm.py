...
def metric(model, features, labels, verbos=False):
    ...
    for ind, cluster in enumerate(model_clusters):
        ...
        if verbos:
            print(
                'In cluster %d there are %d examples, of which %d are pyramidal (%.2f), %d are interneurons (%.2f)'
                ' and %d are untagged (%.2f)' % (ind + 1, total, counter_pyr, 100 * counter_pyr / total, counter_in,
                                                 100 * counter_in / total, counter_ut, 100 * counter_ut / total))
...
def find_elbow(curve):
    ...
    scalarProduct = np.sum(vecFromFirst * np.repeat(np.expand_dims(lineVecNorm, axis=0), nPoints, axis=0), axis=1)
...
def run(use_tsne, tsne_n_components, use_scale, use_pca, pca_n_components, use_ica, ica_n_components,
    ...
    if use_tsne:
        ...
        data_vis = TSNE(n_components=tsne_n_components, perplexity=30, verbose=1, n_iter=2000).fit_transform(
            data_features)
        if verbos:
            plt.scatter(data_vis[data_labels < 0, vis_tsne_feature1], data_vis[data_labels < 0, vis_tsne_feature2],
                        label='unlabeled')
            plt.scatter(data_vis[data_labels == 1, vis_tsne_feature1], data_vis[data_labels == 1, vis_tsne_feature2],
                        label='pyramidal')
            plt.scatter(data_vis[data_labels == 0, vis_tsne_feature1], data_vis[data_labels == 0, vis_tsne_feature2],
                        label='interneuron')
            plt.legend()
    ...
    for covariance_type in covariance_type_options:
        ...
        print('Best BIC with %s covariance type was %.3f, achieved with %d components' % (
            covariance_type, bics_temp[best_n_components - 2], best_n_components))
        best_bic = float('inf')
        avg_errors = 0
        best_model = None
        for i in range(2 * n):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_scale', type=bool, help='scale the data', default=True)
    ...
    parser.add_argument('--saving_path', type=str, help='path to save graphs, assumed to be created',
                        default='../graphs/')
    parser.add_argument('--min_search_components', type=int, help='minimal number of gmm components to check',
                        default=2)
    parser.add_argument('--max_search_components', type=int, help='maximal number of gmm components to check',
                        default=20)
