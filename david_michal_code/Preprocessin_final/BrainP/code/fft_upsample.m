...
% revisions
% 10-mar-14 bug fix: when odd and edges are expanded, there was an offset
...
if edges
    if odd
        %rmv = [ 1 : ( round( m / 3 * f ) - round( f / 3 ) ) ( round( 2 * m / 3 * f + 1 ) - 2 * round( f / 3 ) ) : m * f ];
        rmv = [ 1 : round( m / 3 * f - f / 3 ) round( 2 * m / 3 * f + 1 - 2 * f / 3 ) : m * f ];
    else
