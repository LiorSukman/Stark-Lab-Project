X1 = struct2array(cell2mat(features(1)));
X2 = struct2array(cell2mat(features(2)));
...
y_train = y1(rand_num(1:round(0.8*length(rand_num))),:);

X_test = x(rand_num(round(0.8*length(rand_num))+1:end),:);
y_test = y1(rand_num(round(0.8*length(rand_num))+1:end),:);
