...
def print_statistics_spv():
    cell_class_mat = scipy.io.loadmat("Data\\CelltypeClassification.mat")['sPV']
    options = ['act', 'exc', 'inh']
    total = len(cell_class_mat[options[0]][0][0])
    for field in options:
        count = counter_one(cell_class_mat[field][0][0])
    ...
    for i, field1 in enumerate(options):
        for field2 in options[i + 1:]:
            count = counter_two(cell_class_mat[field1][0][0], cell_class_mat[field2][0][0])
            print(
                'There are %d cells with %s and %s tags which is %.3f%%' % (count, field1, field2, 100 * count / total))
